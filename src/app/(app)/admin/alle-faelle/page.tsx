...
/**
 * Team-weite, dichte Fallübersicht (Admin) - ausgelagert aus /dashboard, das jetzt für alle Rollen
 * inkl. Admin die persönliche "Meine Fälle"-Kartenansicht zeigt (nur eigene/vertretene Fälle).
 */
export default async function AlleFaellePage({
  searchParams,
}: {
  ...
  const [employees, helpTypes, totalOpen, activeCount, completedCount, archivedCount] = await Promise.all([
    ...
    prisma.case.count({ where: { archived: false, status: { not: "COMPLETED" } } }),
    prisma.case.count({ where: { archived: false, status: "ACTIVE" } }),
    prisma.case.count({ where: { archived: false, status: "COMPLETED" } }),
    prisma.case.count({ where: { archived: true } }),
  ]);

  const showArchived = params.archived === "1";
  const where: Prisma.CaseWhereInput = { archived: showArchived };
