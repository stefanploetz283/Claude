...
export type VoiceTimeExtractionResult =
  | {
      ok: true;
      date: string;
      durationMinutes: number;
      activityType: "VERWALTUNG" | "FAHRZEITEN" | "SONSTIGES" | "FALL";
      note: string;
...
/** Analog zu extractServiceEntryFromVoice, aber für die interne Zeiterfassung (kein Jugendamt-Bezug). */
export async function extractTimeEntryFromVoice(transcript: string): Promise<VoiceTimeExtractionResult> {
  const trimmed = transcript.trim();
  if (!trimmed) {
    return { ok: false, error: "Keine Sprache erkannt. Bitte erneut versuchen." };
  }
  ...
  try {
    response = await anthropic.messages.create({
      ...
      tools: [
        {
          name: "extract_time_entry",
          description: "Extrahiert die strukturierten Felder eines internen Zeiterfassungs-Eintrags aus einem Diktat.",
          input_schema: {
            type: "object",
            properties: {
              date: { type: "string", description: "Datum der Tätigkeit im Format YYYY-MM-DD." },
              durationMinutes: { type: "number", description: "Dauer der Tätigkeit in Minuten (aus Uhrzeitangaben oder direkter Dauer berechnen)." },
              activityType: {
                type: "string",
                enum: ["VERWALTUNG", "FAHRZEITEN", "SONSTIGES", "FALL"],
                description: "Art der Tätigkeit. 'FALL', wenn ein konkreter Klient/Fall genannt wird, sonst VERWALTUNG/FAHRZEITEN/SONSTIGES.",
              },
              clientName: { type: "string", description: "Name des Klienten, falls activityType FALL ist, sonst leerer String." },
              note: { type: "string", description: "Kurze sachliche Notiz basierend auf dem Diktat." },
            },
            required: ["date", "durationMinutes", "activityType", "clientName", "note"],
            additionalProperties: false,
          },
          strict: true,
        },
    ...
    console.error("Anthropic-Aufruf für Zeiterfassungs-Diktat fehlgeschlagen:", err);
    return { ok: false, error: "Die Sprachverarbeitung ist fehlgeschlagen. Bitte erneut versuchen." };
  }
  ...
  const toolUse = response.content.find((b) => b.type === "tool_use" && b.name === "extract_time_entry");
  if (!toolUse || toolUse.type !== "tool_use") {
    return { ok: false, error: "Aus dem Diktat konnten keine Daten extrahiert werden. Bitte erneut versuchen." };
  }
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(extracted.date) || !Number.isFinite(extracted.durationMinutes) || extracted.durationMinutes <= 0) {
    return { ok: false, error: "Datum oder Dauer aus dem Diktat konnten nicht eindeutig erkannt werden. Bitte manuell nachtragen." };
  }

  let candidates: VoiceCaseCandidate[] = [];
  let autoSelectedCaseId: string | null = null;
  if (extracted.activityType === "FALL" && extracted.clientName.trim()) {
    const matches = bestMatches(
      extracted.clientName,
      cases,
      (c) => `${c.client.lastName}, ${c.client.firstName}`,
  ...
  return {
    ok: true,
    date: extracted.date,
    durationMinutes: Math.round(extracted.durationMinutes),
    activityType: extracted.activityType,
    note: extracted.note,
    clientNameHeard: extracted.activityType === "FALL" ? extracted.clientName : null,
    autoSelectedCaseId,
    candidates,
  };
}
