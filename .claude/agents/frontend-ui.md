...
4. Match existing component/styling conventions in the repo rather than introducing a new design system ad hoc — check for a shared component library or Tailwind config first. For new views, polish passes, or anything visually non-trivial, invoke the `impeccable`, `emil-design-eng`, or `taste-skill` skills (under `.claude/skills/`) rather than defaulting to generic framework look-and-feel.
