...
6. This project has three design skills installed under `.claude/skills/`: `impeccable`, `emil-design-eng`, and `taste-skill`. Invoke the relevant one(s) for palettes, type pairings, animation, and layout patterns rather than inventing from scratch on every task — but adapt anything generic to fit the practice's specific, warm-but-professional context.
