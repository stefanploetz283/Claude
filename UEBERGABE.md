...
3. **Budgetrechner** (`/finanzen/budgetrechner`, Rolle ADMIN + VERWALTUNG): eigenständiges Modul für die Positionen des Blatts „Weitere Betriebskosten + AfA" der ReKo-Entgeltkalkulation. Frei benannte Budget-Positionen (`BudgetKategorie`) mit Jahresbudget – manuell pflegbar **oder** per `.xlsx`-Import (Vorschau/Bestätigung). Ausgaben (`BudgetAusgabe`) manuell oder per monatlichem Finom-CSV-Import mit Stichwort-Zuordnung + Prüf-Schritt. Dashboard mit Ampel (grün < 70 %, gelb 70–100 %, rot > 100 %), separater Bereich „nicht eingeplante Kosten" (kategorielos + budgetrelevant) als Beleg für ReKo-Nachverhandlungen, PDF-Export. Bewusst getrennt vom Betriebscockpit. Code: `src/lib/budgetrechner/`, `src/app/(app)/finanzen/budgetrechner/`, `src/lib/export/budgetrechner-report.ts`, Migration `20260903120000_budgetrechner`. **Offen:** Der Excel-Parser (`src/lib/budgetrechner/excel-import.ts`) ist generisch und muss noch an das reale Blatt-Layout einer echten Entgeltkalkulation kalibriert werden (Beispieldatei unter `local/`).
